...
def generate_abnormal_event(machine_id,previous_usage,machines,history_stat):
    ...
    param = 1
    if machine_type =='grid_import': param =0.8
    elif previous_usage > 6000: param = 0.25
...
def following_events(date_str,previous_events,topic_name,producer):
    ...
    for pv in previous_events: # = for each machine
        ...
        if current_event['running'] and random.random()>=0.999: # abnormal event 
            current_event['usage'] = generate_abnormal_event(pv['machine_id'],pv['usage'],machines,history_stat) 
        else:
